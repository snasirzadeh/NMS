...
the backend and is useful for display and duplicate detection. The stored text
and checksum are deliberately shaped so a later phase can add diffing without
changing the capture contract; diffing is not implemented here.
